    ...
    <div className="w-[460px] h-full bg-white border-l border-gray-200 flex flex-col overflow-hidden">
      ...
      <div className="shrink-0 flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-100">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Generated plan · Aug 2024</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            aria-label="Download this document"
            className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:border-gray-400 transition-colors cursor-pointer"
          >
            <Download size={14} />
          </button>
          <button
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-6 no-scrollbar">
        ...
        {/* Case Details */}
        <div>
          <p className="text-xs text-gray-500 tracking-wider mb-3">Case Details</p>
        ...
        {/* Staffing */}
        <div>
          <p className="text-xs text-gray-500 tracking-wider mb-3">Staffing</p>
          <div className="flex flex-col gap-2">
            {STAFF.map((s) => (
        ...
        {/* Parts */}
        <div>
          <p className="text-xs text-gray-500 tracking-wider mb-3">Parts & Materials</p>
          <div className="bg-gray-50 rounded-xl overflow-hidden border border-gray-100">
            <div className="grid grid-cols-[1fr_auto_auto] gap-x-4 px-4 py-2 border-b border-gray-200">
              ...
              <span className="text-xs text-gray-500 text-right">Status</span>
            </div>
            {PARTS.map((p, i) => (
              <div key={p.name} className={`grid grid-cols-[1fr_auto_auto] gap-x-4 px-4 py-2.5 items-center ${i < PARTS.length - 1 ? "border-b border-gray-100" : ""}`}>
                ...
                <span className={`text-xs text-right whitespace-nowrap ${p.status === "PO pending" ? "text-gray-800 font-medium" : "text-gray-500"}`}>
                  {p.status}
                </span>
              </div>
            ))}
        ...
        {/* Schedule */}
        <div>
          <p className="text-xs text-gray-500 tracking-wider mb-3">Schedule</p>
          <div className="bg-gray-50 rounded-xl px-4 py-3 border border-gray-100">
            <p className="text-sm text-gray-700">5 – 16 August 2024</p>
            <p className="text-xs text-gray-500 mt-1">10 working days · Central US</p>
          </div>
        </div>
